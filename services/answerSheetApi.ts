  ...
  if (hasNewFormat) {
    ...
    for (const q of questionEvaluations) {
      ...
      if (isSubjective) {
        ...
        mcqResults.push({
          ...
          questionNumber: q.questionNumber ?? q.QuestionNumber ?? 0,
          questionText: q.questionText || q.QuestionText || '',
          selectedOption: q.extractedAnswer || q.ExtractedAnswer || q.studentAnswer || q.StudentAnswer || q.selectedOption || q.SelectedOption || '',
    ...
    for (const m of mcqEvaluations) {
      mcqResults.push({
        ...
        questionNumber: m.questionNumber ?? m.QuestionNumber ?? 0,
        questionText: m.questionText || m.QuestionText || '',
        selectedOption: m.extractedAnswer || m.ExtractedAnswer || m.selectedOption || m.SelectedOption || m.studentAnswer || m.StudentAnswer || '',
        correctAnswer: m.correctAnswer || m.CorrectAnswer || m.correctOption || m.CorrectOption || m.modelAnswer || m.ModelAnswer || '',
        isCorrect: m.isCorrect ?? m.IsCorrect ?? false,
